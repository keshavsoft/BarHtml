...
let LocalDateChange = ({ inHeadData }) => {
    let jVarLocalReturnData = JSON.parse(JSON.stringify(inHeadData));

    if ("Date" in jVarLocalReturnData) {
        jVarLocalReturnData.Date = DateFuncs.ddMMyyyy({ inDate: jVarLocalReturnData.Date });
    };
...
let PrintOriginalBillWithDuplicate = async (inPk) => {
    try {
        //  let jVarLocalReturnArray = await LocalFirmHeading();
        let jVarLocalReturnArray = [];

        let LocalSalesDataAsJson = await SalesShowFunc();
        // console.log("LocalSalesData : ", LocalSalesData);

        let LocalSalesItemsData = await SalesItemsShowFunc();

        let LocalSalesItemsDataNeeded = Object.values(LocalSalesItemsData).filter(element => {
            return element.FK === parseInt(inPk);
        });
        //let LocalSalesDataAsJson = JSON.parse(LocalSalesData);
        let LocalDataNeeded = LocalSalesDataAsJson[inPk];
        LocalDataNeeded = LocalDateChange({ inHeadData: LocalDataNeeded });

        //orignial bill
        jVarLocalReturnArray.push(... await BillPrintCommonFuncs.jFHeadDetails({ inHeadData: LocalDataNeeded }));
        jVarLocalReturnArray.push(... await BillPrintCommonFuncs.InvGrid.jFLocalItemHeading());
        jVarLocalReturnArray.push(... await BillPrintCommonFuncs.InvGrid.LoopItemsArray({ inItemArray: LocalSalesItemsDataNeeded }));
        jVarLocalReturnArray.push(... await BillPrintCommonFuncs.CutterInsert());

        //  jVarLocalReturnArray.push(...await BillPrintCommonFuncs.ForReprint.FirmHeading());
        ...
        await BillPrintCommonFuncs.PrepareTextFileFromArray({ inDataArray: jVarLocalReturnArray, inCharactersPerLine: 42 });

        await BillPrintCommonFuncs.PrintToPrinter();
    } catch (error) {
...
export { PrintOriginalBillWithDuplicate }
